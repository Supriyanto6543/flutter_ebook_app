...
class _EbookSearchState extends State<EbookSearch> {
  ...
  Future fetchSearchData(String str) async{
    var response = await Dio().get(ApiConstant().baseUrl+ApiConstant().api+ApiConstant().latest, options: Options(
    ...
    for(Map<String, dynamic> sliderData in response.data){
      listNewEbook.add(ModelEbook(id: sliderData['id'],
...
class MangaSearchAction extends SearchDelegate<String>{
  ...
  Future getDataFromJson(String str) async{
    var response = await http.get(Uri.parse(ApiConstant().baseUrl+ApiConstant().api+ApiConstant().search+str),
  ...
  List<Widget> buildActions(BuildContext context) {
    return [IconButton(icon: Icon(Icons.clear, color: Colors.black,), onPressed: (){
      query = "";
    },)];
  }
  ...
  Widget buildSuggestions(BuildContext context) {
    return query.isEmpty ? Center(child: Text('Get search Ebook', style: TextStyle(
        fontSize: 18,
        color: Colors.black),),) :
    FutureBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if(!snapshot.hasData){
          ...
          for(Map<String, dynamic> sliderData in jsonDecode(snapshot.data)){
            ebook.add(ModelEbook(id: sliderData['id'],
                title: sliderData['title'],
                photo: sliderData['photo'],
                description: sliderData['description'],
                catId: sliderData['cat_id'],
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text('${ebook[index].title}', style: TextStyle(
                                          color: Colors.black,
                                          fontSize: 17, fontWeight: FontWeight.w500),
                                      ),
                                      SizedBox(height: 1.5.h,),
                                      Text('Author: ${ebook[index].authorName}', style: TextStyle(
                                          color: Colors.black,
                                          fontSize: 15, fontWeight: FontWeight.w400), maxLines: 5, overflow: TextOverflow.ellipsis,),
                                      SizedBox(height: 0.5.h,),
                                      Text('Publisher: ${ebook[index].publisherName}', style: TextStyle(
                                          color: Colors.black,
                                          fontSize: 15, fontWeight: FontWeight.w400), maxLines: 5, overflow: TextOverflow.ellipsis,),
                                    ],
                                  ),
